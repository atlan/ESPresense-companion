<script lang="ts">
	...
	import { editMode, selectedNodeId, nodeEdits, placingNode, pendingNode, pickingWalkPoint, selectedRoomId, roomEdits, draftRoom, traceImage, imageTool, scalePoints, boundsPoints, resetEditState } from '$lib/floorplanEdit';
...
<!-- Right-aligned below the background-image icon (fixed top-4 right-4); top-16 also clears
     the centered floor-tabs header, which overlapped a top-left toolbar at phone widths. -->
<div class="absolute top-16 right-2 z-10 flex flex-col items-end gap-2 w-80 max-w-[90vw]">
	...
	{#if $editMode === 'off'}
		<button
			class="btn-icon shadow {$pickingWalkPoint ? 'preset-filled-warning-500' : 'preset-tonal'}"
			onclick={() => ($pickingWalkPoint = !$pickingWalkPoint)}
			title={$pickingWalkPoint ? 'Click the map to pick the walk test spot (click again to cancel)' : 'Pick a walk test spot on the map'}
			aria-label="Pick walk test spot"
		>
