	...
	<nav class="h-50 text-black">
		<div class="flex items-center space-x-4">
			<div class="flex bg-slate-600 rounded-full p-1">
				...
				<button class="px-4 py-1 rounded-full text-sm font-medium transition-colors {calibrationType === 'setup' ? 'bg-emerald-400 text-black' : 'text-white hover:bg-slate-500'}" onclick={() => (calibrationType = 'setup')}>
					Setup
				</button>
			</div>
		</div>
